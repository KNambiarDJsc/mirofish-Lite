...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    """
    创建LLM模型，支持 OpenAI 和 Gemini 平台原生集成
    
    Args:
        config: 模拟配置字典
        use_boost: 是否使用加速 LLM 配置
    """
    # 检查是否有加速配置
    ...
    if is_groq:
        ...
        # 兼容旧版本可能使用的 GOOGLE_API_KEY
        os.environ["GOOGLE_API_KEY"] = os.environ["GEMINI_API_KEY"]
        platform_label = "GEMINI"
    else:
        platform = ModelPlatformType.OPENAI
        os.environ["OPENAI_API_KEY"] = llm_api_key or openai_key or ""
        if llm_base_url:
...
async def run_twitter_simulation(
    ...
    if action_logger:
        try:
            ...
            log_info(f"警告: 记录启动事件失败: {e}")
    
    # 验证环境和配置
    if not os.environ.get("LLM_API_KEY") and not os.environ.get("GEMINI_API_KEY"):
        log_info("错误: 未配置 LLM_API_KEY 或 GEMINI_API_KEY")
        return result
    ...
    for round_num in range(total_rounds):
        ...
        log_info(f"第 {round_num + 1} 轮: 正在处理 {len(active_agents)} 个活跃 Agent 的动作...")
        if action_logger:
            action_logger.log_heartbeat(round_num + 1, f"Processing {len(active_agents)} agents")
            
        try:
            # 为每轮动作设置超时，防止 LLM 请求无限挂起
            await asyncio.wait_for(result.env.step(actions), timeout=300)
            log_info(f"第 {round_num + 1} 轮: LLM 动作处理完成")
        except asyncio.TimeoutError:
            log_info(f"警告: 第 {round_num + 1} 轮主循环超时 (300s)，跳过本轮动作等待")
            if action_logger:
                action_logger.log_heartbeat(round_num + 1, "timeout_skipped")
        except Exception as e:
            log_info(f"错误: 第 {round_num + 1} 轮执行出错: {e}")
            if action_logger:
                action_logger.log_heartbeat(round_num + 1, f"error: {str(e)}")
...
async def main():
    ...
    try:
        ...
        import traceback
        error_msg = f"模拟启动过程发生严重错误: {e}\n{traceback.format_exc()}"
        log_manager.error(error_msg)
        # 确保错误被写入特定的动作日志，以便前端能检测到
        if twitter_logger: twitter_logger.error(f"STARTUP_ERROR: {e}")
        if reddit_logger: reddit_logger.error(f"STARTUP_ERROR: {e}")
        # 延迟退出，让监控线程有机会读取到这个错误日志
