...
class TwitterSimulationRunner:
    ...
    async def run(self, max_rounds: int = None):
        ...
        self.env = oasis.make(
            ...
            semaphore=2,  # 限制最大并发 LLM 请求数，防止 API 过载 (Gemini Free Tier 优化)
        )
        ...
        for round_num in range(total_rounds):
            ...
            # 执行动作
            print(f"  [Round {round_num + 1}] 正在处理 {len(active_agents)} 个 Agent 的动作...")
            self.action_logger.log_heartbeat(round_num + 1, f"Processing {len(active_agents)} agents")
            
            try:
                # 为每轮动作设置超时，防止 LLM 请求无限挂起
                await asyncio.wait_for(self.env.step(actions), timeout=300)
            except asyncio.TimeoutError:
                print(f"  警告: 第 {round_num + 1} 轮主循环超时 (300s)，跳过本轮动作等待")
                self.action_logger.log_heartbeat(round_num + 1, "timeout_skipped")
            except Exception as e:
                print(f"  错误: 第 {round_num + 1} 轮执行出错: {e}")
                self.action_logger.log_heartbeat(round_num + 1, f"error: {str(e)}")
            
            # 记录动作到日志
            round_actions_count = 0
            for _, agent in active_agents:
                # 记录 Agent 参与了本轮
                self.action_logger.log_action(
                    round_num=round_num + 1,
                    agent_id=agent.id,
                    agent_name=getattr(agent, 'name', f"Agent_{agent.id}"),
                    action_type="LLM_ACTION_STEP",
                    success=True
                )
                round_actions_count += 1
            
            # 记录轮次结束
            self.action_logger.log_round_end(round_num + 1, round_actions_count)
        ...
        # 记录模拟结束
        # 统计总动作数：由于 standalone 脚本目前没做复杂的数据库轮询，动作数先记为 0 或者忽略
