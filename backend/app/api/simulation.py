...
def generate_profiles():
    ...
    try:
        ...
        # Parallel profile generation logic
        profiles = []
...
def start_simulation():
    ...
    try:
        ...
        if not Config.LLM_API_KEY:
            logger.error("无法启动模拟: LLM_API_KEY (或 GEMINI_API_KEY) 未配置")
            return jsonify({
                "success": False,
                "error": "No LLM provider configured. Please set GEMINI_API_KEY in your .env file."
            }), 400
