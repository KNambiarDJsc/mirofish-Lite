...
# Stub result classes to keep existing code working
class SearchResult:
    def __init__(self, facts=None, nodes=None): self.facts = facts or []; self.nodes = nodes or []
...
class SupabaseSearchService:
    """Supabase-backed search service replacing ZepToolsService."""
    def __init__(self):
    ...
    def insight_forge(self, graph_id: str, query: str, simulation_requirement: str = "", report_context: str = "") -> SearchResult:
        # Enhanced search: combine edge facts + node summaries
        edges = self.memory.search_edges(graph_id, query, top_k=15)
        nodes = self.memory.search_nodes(graph_id, query, top_k=5)
        facts = [e.get("fact", "") for e in edges if e.get("fact")]
        node_summaries = [f"{n.get('name', '')}: {n.get('summary', '')}" for n in nodes if n.get("summary")]
        return SearchResult(facts=facts + node_summaries)

    def panorama_search(self, graph_id: str, query: str, include_expired: bool = True) -> SearchResult:
    ...
    def get_simulation_context(self, graph_id: str, simulation_requirement: str = "") -> dict:
        ...
        # 获取一些初始事实作为样本
        related_search = self.insight_forge(graph_id, simulation_requirement or "all events")
        related_facts = related_search.facts[:15]
...
═══════════════════════════════════════════════════════════════
    ...
    - 每次报告生成必须调用工具获取实时模拟数据
    - 每个章节调用工具 1 次来观察模拟的世界，它是预测未来的核心依据
    - 保持简洁，直接给出最有力的证据和预测结论
...
class ReportAgent:
    ...
    # 最大工具调用次数（每个章节）- Gemini Free Tier 优化
    ...
    def __init__(
        ...
        # Use Supabase search service (replaces ZepToolsService)
